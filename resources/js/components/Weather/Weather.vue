...
export default {
    ...
    methods: {
        fetchCoordinates () {
            return fetch(
                `${this.coordinatesUrl}q=${this.city}&appid=${this.appid}`
            )
                .then(res => res.json())
                .then(data => {
        ...
        fetchData () {
            this.fetchCoordinates().then(data => {
                ...
                fetch(
                    `${this.forecastUrl}lat=${LAT}&lon=${LON}&units=metric&cnt=12&appid=${API_KEY}`
                )
                    .then(res => res.json())
                    .then(data => {
                        this.weatherData = data.list
                        for (let i = 0; i < this.weatherData.length; i++) {
                            this.weatherData[i].hour = new Date(
                                this.weatherData[i].dt * 1000
                            ).getHours()
                            this.weatherData[
                                i
                            ].img = ` https://openweathermap.org/img/wn/${this.weatherData[i].weather[0].icon}@4x.png`
                        }
